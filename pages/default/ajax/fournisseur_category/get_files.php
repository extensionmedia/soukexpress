...
if(file_exists($filesDirectory)){
	...
	foreach(scandir($filesDirectory) as $k=>$v){
		...
		if($v <> "." and $v <> ".." and strpos($v, '.') !== false){
			...
			$images .= "<button class='btn btn-small btn-default'><i class='fas fa-thumbtack'></i> Default</button><button class='btn btn-small btn-red delete_file category' value='". $filesDirectory.$v."'><i class='fas fa-trash-alt'></i> Supp.</button><button class='btn btn-small btn-orange rotate category' value='". $filesDirectory.$v."'><i class='fas fa-sync'></i> Rotate</button>";
			$images .= "</div>";
			$nbr++;
		}
