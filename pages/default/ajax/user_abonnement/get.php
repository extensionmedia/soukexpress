...
if(file_exists($core.$table_name.".php")){
	...
	$current = $p_p *  $_POST['data']['current'];
		...
		if($request == ""){
			$values = $ob->find(null,array("order"=>$sort_by,"limit"=>array($current,$p_p)),"v_course_of_user");
			$totalItems = $ob->getTotalItems();
		}else{
			$totalItems = count($ob->find(null,array("conditions"=>array("LOWER(article_category) like "=>"%".$request."%"),"order"=>$sort_by),"v_course_of_user"));
			$values = $ob->find(null,array("conditions"=>array("LOWER(article_category) like "=>"%".$request."%"),"order"=>$sort_by,"limit"=>array($current,$p_p)),"v_course_of_user");
		}
