...
if(file_exists($core.$table_name.".php")){
	...
	if($request !== ""){$conditions["title like "] = "%".$request."%";}
...
//var_dump($conditions);
	...
	if(empty($conditions)){
		$values = $ob->find(null,array("order"=>$sort_by,"limit"=>array($current,$p_p)),"creative");
		$totalItems = $ob->getTotalItems();
	}else{
		if(count($conditions)>1){
			$totalItems = count($ob->find(null,array("conditions AND"=>$conditions,"order"=>$sort_by),"creative"));
			$values = $ob->find(null,array("conditions AND"=>$conditions,"order"=>$sort_by,"limit"=>array($current,$p_p)),"creative");			
		}else{
			$totalItems = count($ob->find(null,array("conditions"=>$conditions,"order"=>$sort_by),"creative"));
			$values = $ob->find(null,array("conditions"=>$conditions,"order"=>$sort_by,"limit"=>array($current,$p_p)),"creative");			
		}
