...
<div class="row page_title">
	<div class="col_6-inline icon">
		<i class="fas fa-folder-open"></i> Client <span style="font-size: 9px"><?= ($action === "edit")? "<span style='color:red'>" . $data["_md5"] . "</span>" : substr($formToken,0,8) ?></span>
	</div>
				...
				<div class="row" style="margin-bottom: 20px">
					<div class="col_12">
						<div class="btn-group btn-group-radio">
							<?php foreach($ob->find("", array("conditions"=>array("status="=>1)), "article_tarif") as $kk=>$vv){ 
								$checked = "";
								if($action === "edit"){
									if($vv["id"] === $data["id_article_tarif"]){ $checked = "checked";}
								}else{
									if($vv["is_default"]){ $checked = "checked";}
								}
							?>
							<button class="btn btn-default <?= $checked ?>"><?= $vv["article_tarif"] ?></button>
							<?php } ?>
						</div>
					</div>						
				</div>
