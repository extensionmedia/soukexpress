...
<div class="row page_title">
	<div class="col_6-inline icon">
		<i class="fas fa-folder-open"></i> Marque <span style="font-size: 9px"><?= ($action === "edit")? "<span style='color:red'>" . $data["UID"] . "</span>" : substr($formToken,0,8) ?></span>
	</div>
...
<div class="panel">
	...
	<div class="panel-header" style="padding: 0px">
		<div class="panel-header-tab ">
			...
			<a  href="" class="show_files marque" data="<?= ($action === "edit")? ($data["UID"] !== "")? $data["UID"] : substr($formToken,0,8) : substr($formToken,0,8) ?>"><i class="fas fa-images"></i> Images</a>
		</div>
	</div>
		...
		<div class="tab-content">
			...
			<?= ($action === "edit")? "<input class='form-element' type='hidden' id='id' value='".$id."'>" : "" ?>
				...
				<div class="row" style="margin-bottom: 20px">
					<div class="col_6">
						<label for="article_marque">Marque</label>
						<input class="form-element required" type="text" placeholder="Catégorie" id="article_marque" value="<?= ($action === "edit")? $data["article_marque"] : "" ?>">
					</div>	

					<div class="col_6">
						<label for="article_marque_ar">الإسم التجاري</label>
						<input class="form-element required" type="text" placeholder="الإسم التجاري" id="article_marque_ar" value="<?= ($action === "edit")? $data["article_marque_ar"] : "" ?>">
					</div>	
				</div>
				<div class="row" style="margin-bottom: 20px">
					<div class="col_6">
		...
		<div class="tab-content" style="display: none" >
			<div class="row upload">
				...
				<div class="col_8-inline" style="text-align: right; padding-top: 10px">
					...
					<input type="file" id="upload_file_marque" data="<?= ($action === "edit")? $data["UID"] : substr($formToken,0,8) ?>" class="" name="image" capture style="position: absolute; z-index: 9999; top: 0; left: 0; background-color: aqua; padding: 10px 0; opacity: 0">
					</button>	
					<button class="btn btn-blue show_files marque" value="<?= ($action === "edit")? $data["UID"] : substr($formToken,0,8) ?>"> Actualiser </button>					
				</div>
