...
?>
	...
	<div class="row page_title">
		<div class="col_6-inline icon">
			<i class="fas fa-address-card"></i> Creative
		</div>
	...
	<div class="row <?= strtolower($table_name) ?>">
		...
		<?php $values = $ob->find(null,array("order"=>"id ASC","limit"=>array(0,20)),"creative");  
								...
								<?php
									foreach($columns as $key=>$value){
										if(isset($v[ $columns[$key]["column"] ])){
											...
											}elseif($columns[$key]["column"] == "title"){
												echo "<td>" . substr($v[ $columns[$key]["column"] ], 0, 200) . "</td>";
											}else{
												echo "<td>" . $v[ $columns[$key]["column"] ] . "</td>";
											}											
										}else{
											if($columns[$key]["column"] == "client"){
												echo "<td>" . $v["first_name"]. " ". $v["last_name"] . "</td>";	
											}elseif($columns[$key]["column"] == "contact"){
												echo "<td>" . $v["phone_1"]. " ". $v["phone_2"] . "</td>";	
											}else{
												echo  "<td>NaN</td>";
											}
