...
class Article_Sous_Category extends Modal{
	...
	private $columns = array(
		...
		array("column" => "article_sous_category_fr", "label"=>"SOUS CATEGORIE (fr)", "style"=>"min-width:125px;font-weight:bold"),
		array("column" => "article_sous_category_es", "label"=>"SOUS CATEGORIE (es)", "style"=>"min-width:125px;font-weight:bold"),
		array("column" => "article_sous_category_ar", "label"=>"SOUS CATEGORIE (ar)", "style"=>"min-width:125px;font-weight:bold"),
		array("column" => "ord", "label"=>"ORDER", "style"=>"min-width:125px;font-weight:bold"),
	...
	public function drawTable($args = null, $conditions = null, $useTableName = null){
		...
		if($style === "list"){
			...
			foreach($values as $k=>$v){
				$returned .= '					<tr class="edit_ligne '.$sub.'" data-page="'.$_t.'">';
				foreach($columns as $key=>$value){
					...
					if(isset($v[ $columns[$key]["column"] ])){
						...
						if($columns[$key]["column"] == "actions"){
							$returned .=   "<td style='".$style."'><button style='margin-right:10px' data-page='".$_t."' class='btn btn-red remove_ligne' value='".$v["id"]."'><i class='fas fa-trash-alt'></i></button><button data-page='".$_t."' class='btn btn-orange edit_ligne ".$sub."' value='".$v["id"]."'><i class='fas fa-edit'></i></button></td>";												
			...
			foreach($values as $k=>$v){
				$returned .= "<div style='position:relative; width:180px; height:180px; margin:5px; display:inline-block; background-color:rgba(220,220,220,0.5); border:1px solid #C0C0C0; border-radius:7px'>" . $this->get_files("sous_category", (is_null($v["UID"]))? "a": ($v["UID"] === "")? "a": $v["UID"],array("id"=>$v["id"])) . "<span class='label label-red' style='position:absolute; top:10px; left:10px'>".$v["article_sous_category_fr"] . "</span></div>";
			}
