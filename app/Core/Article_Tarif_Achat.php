...
class Article_Tarif_Achat extends Modal{
	...
	private $tableName = "Article_Tarif_Achat";
...
// construct
	...
	public function drawTable($args = null, $conditions = null, $useTableName = null){
		...
		$values = $this->find(null,$conditions,"v_article_tarif_achat");
		$style = "";	
		...
		foreach($values as $k=>$v){
			$returned .= '					<tr class="edit_tarif_achat" data-page="'.$_t.'">';
			foreach($columns as $key=>$value){
				...
				if(isset($v[ $columns[$key]["column"] ])){
					...
					if($columns[$key]["column"] == "actions"){
						$returned .=   "<td style='".$style."'><button style='margin-right:10px' data-page='".$_t."' class='btn btn-red remove_ligne tarif_achat' value='".$v["id"]."'><i class='fas fa-trash-alt'></i></button><button data-page='".$_t."' class='btn btn-orange edit_tarif_achat' value='".$v["id"]."'><i class='fas fa-edit'></i></button></td>";												
...
$article_tarif_achat = new Article_Tarif_Achat;