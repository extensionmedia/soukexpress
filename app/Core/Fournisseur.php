...
class Fournisseur extends Modal{
	...
	private $columns = array(
		...
		array("column" => "phone", "label"=>"PHONE", "style"=>"width:145px;min-width:125px;font-weight:bold; font-size:14px"),
		array("column" => "email", "label"=>"EMAIL", "style"=>"width:225px;min-width:125px;font-weight:bold"),
		array("column" => "city", "label"=>"VILLE", "style"=>"width:125px;min-width:105px; width:100px;"),
				...
				if(isset($v[ $columns[$key]["column"] ])){
					if($columns[$key]["column"] == "id"){
						...
						if($v["notes"] !== "" and isset($v["notes"])){
							$returned .= "<td style='".$style."'>" . $v["fournisseur_name"] . " <span style='color:blue; font-size:12px'><i class='fas fa-info-circle'></i></span></td>";
						}else{
							$returned .= "<td style='".$style."'>" . $v["fournisseur_name"] . "</td>";
						}
					}else{
					...
					if($columns[$key]["column"] == "phone"){
						
						$phone = '<i class="fas fa-phone-square"></i> ' . $v["phone_1"];
						if(!is_null($v["phone_2"]) && $v["phone_2"] !== ""){
							$phone .=  '<br><i class="fas fa-phone-square"></i> ' . $v["phone_2"];
						}
							
							
						$returned .=  "<td style='".$style."'>" . $phone . "</td>";
