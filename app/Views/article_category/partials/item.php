<div data-lavel="<?= $lavel ?>" data-parent_id="<?= $parent ?>" data-id="<?= $category["id"] ?>" 
    class="load_sous_category relative flex items-center mb-2 border rounded hover:bg-gray-100 cursor-pointer item">
    ...
    <div class="text-xs">
        <div class="text-sm font-bold"><?= $category["article_category_ar"] ?></div>
        <div><?= $category["article_category_fr"] ?></div>
        <div><?= $category["article_category_es"] ?></div>
    </div>
    <div class="absolute top-0 right-0 m-1 rounded bg-gray-500 text-gray-100 text-xs px-1 hide"><?= $category["id"] ?></div>
