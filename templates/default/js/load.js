...
$(document).ready(function(){
	...
	***************************/	
	...
	$(document).on('change', '#article_category, #id_article_category', function(){
			...
			*/
			if($("#article_sous_category").length>0){
				$("#article_sous_category").prepend("chargement...!");
				$("#article_parent").html("<option value='-1'> Sous Categorie </option>");						
			}else{
				$("#id_article_sous_category").prepend("chargement...!");
			...
			$.ajax({
				...
				if (response.code === 1){
					if($("#article_sous_category").length>0){
						$("#article_sous_category").html(response.msg);
						$("#article_parent").html("<option value='-1'> Sous Categorie </option>");	
						
					}else{
						$("#id_article_sous_category").html(response.msg);
						$("#article_parent").html("<option value='-1'> Sous Categorie </option>");						
					}
					$(".content .loading").remove();
				}else{
					$(".content .loading").remove();
					$(".debug").html('	<div class="info info-error info-dismissible"> <div class="info-message"> '+response+' </div> <a href="#" class="close" data-dismiss="info" aria-label="close">&times;</a></div>');
				}
	...
	$(document).on('change', '#article_sous_category, #id_article_sous_category', function(){
		
		var id_category = ($("#article_category").length>0)? $("#article_category").val(): $("#id_article_category").val();
		var id_sous_category = $(this).val();
		var data = {
			'module' 	 		: 	'sous_category',
			'id_category'		:	id_category,
			'id_sous_category'	:	id_sous_category
		};
		console.log(data);
		...
		$.ajax({
			...
			if (response.code === 1){
				$("#article_parent").html(response.msg);
